...
class AccountController extends Controller
{
	...
	public function actionReferences(){
		...
		if(isset($_POST['UsersReferences'])){
			...
			$userBizData = $userBizLocationData->attributes;
			unset($userBizData['id']);
			unset($userBizData['user_id']);

			$userRefData = array();
			$userReferencesData = UsersReferences::model()->find('user_id='.$user_id);	
			if(!empty($userReferencesData)){
				$userRefData = $userReferencesData->attributes;
				unset($userRefData['id']);
				unset($userRefData['user_id']);			
			}
			$apiData	=	array_merge($userBusinessData,$userPersonalData,$userBizData,$userRefData);		
			CommonMethods::dataSubmissionToVelocify($apiData);
...
<tr>
  ...
  <tr><td ><table width="950" ><tr>
    ...
    <td width="17%" style="border-bottom:1px solid #000;">&nbsp;</td>
    <td width="31%" align="center">Average Processing Volume</td>
    <td width="17%" style="border-bottom:1px solid #000;">&nbsp;</td></table></td></tr>
  </tr>'; 
...
//exit;
	...
	public function actionDocusignresponse(){
		...
		if($event == 'signing_complete'){
			...
			$templateContent = "Hello ".$agentModel->fname.",<br /><br /> ".$model->fname." ".$model->lname." with ".$model->business_name." has completed and signed the online application. An email will arrive shortly from your team leader with the application attached.";

/*			Please click the following link to find the PDF in google drive. The file name is ".str_replace(' ','_',$usersBusinessData->legal_name).".pdf<br />";
	...
	public function actionUploads(){
		...
		if(isset($_FILES['UsersFinancialsDocuments'])){
            ...
            if (isset($documents) && $documentCounter > 0) { 
                ...
                foreach ($documents as $doc => $file) {
					...
					$fileName = str_replace(' ','_',$fileName);
					$time = time();
					$filePath = 'upload/financials/'.$time.'_'.$fileName;
					$fileType = CommonMethods::getFileExtension($file);
					//echo Yii::getPathOfAlias('webroot').'/'.$filePath;exit;
                    if ($file->saveAs(Yii::getPathOfAlias('webroot').'/'.$filePath)) {
                        // add it to the main model now
                        $modelDocuments = new UsersFinancialsDocuments();
                        $modelDocuments->filename = $file->name; //filename is just what I chose to call it in my model
                        $modelDocuments->user_id = $user_id; 
                        $modelDocuments->filepath = 'upload/financials/'.$time.'_'.$fileName; 
                        $modelDocuments->file_type = $fileType ; 
						$modelDocuments->save();
						//echo "<pre>";print_r($modelDocuments);exit;
                    }                    
                }
		...
		$model = new UsersFinancialsDocuments();
		$user_id = Yii::app()->user->getState('user_id');
		$userData = Users::model()->findByPk($user_id);
		$this->userModel = $userData;
		
		//$this->render('uploads',array('model'=>$model,'userData'=>$userData));
		$this->render('uploads',array('userData'=>$userData,'model'=>$model));
	}
}