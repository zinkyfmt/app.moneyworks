...
//advanced - пользователь сам определяет тулбар
    ...
    // to make the content look like if it were in your target page
		...
		$options['filebrowserBrowseUrl'] = $base_url.'/protected/extensions/ckeditor/assets/kcfinder/browse.php?type=files';
        $options['filebrowserImageBrowseUrl'] = $base_url.'/protected/extensions/ckeditor/assets/kcfinder/browse.php?type=images';
       	$options['filebrowserFlashBrowseUrl'] = $base_url.'/protected/extensions/ckeditor/assets/kcfinder/browse.php?type=Flash';
        $options['filebrowserUploadUrl'] = $base_url.'/protected/extensions/ckeditor/assets/kcfinder/upload.php?type=files';
        $options['filebrowserImageUploadUrl'] = $base_url.'/protected/extensions/ckeditor/assets/kcfinder/upload.php?type=images';
        $options['filebrowserFlashUploadUrl'] = $base_url.'/protected/extensions/ckeditor/assets/kcfinder/upload.php?type=flash';
	  
	   $options['allowedContent']=$this->allowedContent;
...
editor = CKEDITOR.replace('{$name}',{$options});

EOP;

        $cs->registerScript('Yii.'.get_class($this).'#'.$id, $js, CClientScript::POS_LOAD);
// $cs->registerScript('Yii.'.get_class($this).'#'.$id, "jQuery(document).ajaxComplete(function(event){".$js." });");
        if($this->hasModel()) {
