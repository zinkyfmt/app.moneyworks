...
class Paypal extends CComponent{
    ...
    */
    ...
    public function SetExpressCheckout($paymentInfo=array()){ 
        ...
        $amount = urlencode($paymentInfo['Order']['theTotal']);
		$discount = - $paymentInfo['Order']['discount'];
		
		$prodStr = ''; $i=0;
		foreach($paymentInfo['Order']['product'] as $product){
			$prodStr	.=	'&L_PAYMENTREQUEST_0_NAME'.$i.'='.urlencode($product['name']).								
				'&L_PAYMENTREQUEST_0_AMT'.$i.'='.urlencode($product['amount']).
				'&L_PAYMENTREQUEST_0_QTY'.$i.'='. urlencode($product['quantity']);
			$i++;	
		}
		if($paymentInfo['Order']['discount'] > 0){
			$prodStr .=	'&L_PAYMENTREQUEST_0_NAME'.$i.'=Voucher Discount'.
        ...
        */
        ...
        $nvpstr='&PAYMENTREQUEST_0_AMT='.$amount.'&PAYMENTREQUEST_0_ITEMAMT='.$amount.'&PAYMENTREQUEST_0_PAYMENTACTION='.$paymentType.'&PAYMENTREQUEST_0_CURRENCYCODE='.$currencyCode.'&RETURNURL='.$returnURL.'&CANCELURL='.$cancelURL.$prodStr;
    ...
    public function DoExpressCheckoutPayment($paymentInfo=array()){ 
        ...
        $nvpstr='&TOKEN='.urlencode($paymentInfo['TOKEN']).'&PAYERID='.urlencode($paymentInfo['PAYERID']).'&PAYMENTACTION='.urlencode($paymentType).'&AMT='.urlencode($amount).'&CURRENCYCODE='.$currencyCode.'&IPADDRESS='.urlencode($serverName); 
